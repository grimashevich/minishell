...
function compile_files {
	local LIBFT=../library/libft.a
	local LIBTEST=libtest/library/libtest.a
	local CPPFLAGS="-I libtest/include -I ../include"
	local SOURCE="$CPPFLAGS $LIBFT $LIBTEST $FUNCTION/parser.c $FUNCTION/remove_all.c"
...
function function_test {
	printf "\x1b[40m $FUNCTION \x1b[0m\n\t\t"
	for ARGV in $(cat $FUNCTION/list_of_tests.txt)
	do
